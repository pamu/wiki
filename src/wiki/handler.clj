(ns wiki.handler
  (:require [compojure.core :refer [defroutes routes wrap-routes]]
            ...
            [wiki.middleware :as middleware]
            [wiki.session :as session]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "wiki.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (session/start-cleanup-job!)
  (timbre/info "\n-=[ wiki started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "wiki is shutting down...")
  (stop-nrepl)
